...
state, localStorage, or memory. One zod schema per place derives the
server-side read, canonical links, salvage of stale data, and a client
subscription with exact per-key change detection. Links and GET forms work
before JavaScript loads; imperative URL updates ride the Navigation API.
