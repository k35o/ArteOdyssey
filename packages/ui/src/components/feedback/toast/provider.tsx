...
> = ({ children, portalRef = null, position = 'fixed' }) => {
  ...
  // 描画先は DOM がある時点でしか決まらない。document.body は SSR では読めず、
  // portalRef は指す要素がマウントされて初めて埋まる。どちらもレンダー中には
  // 読まず、マウント後に state へ移す。
  ...
  // ref は commit 時に付くので、この effect が走る時点では埋まっている。
  useEffect(() => {
